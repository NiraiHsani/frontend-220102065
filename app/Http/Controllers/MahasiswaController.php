...
class MahasiswaController extends Controller
{
    public function index()
    {
        ...
        $mahasiswas = json_decode($response->body());
    ...
    public function store(Request $request)
    {
        ...
        $response = Http::asForm()->post('http://localhost:8080/mahasiswa', $data);

        if ($response->successful()) {
            return redirect()->route('mahasiswa.index')->with('success', 'kelas created successfully.');
        } else {
    ...
    public function update(Request $request, $npm)
    {
        ...
        if ($response->successful()) {
            return redirect('mahasiswa.index')->with('success', 'Data berhasil diperbarui!');
        } else {
    ...
    public function destroy($npm)
    {
        ...
        if ($response->successful()) {
            return redirect('mahasiswa.index')->with('success', 'Data berhasil dihapus!');
        } else {
            return redirect('mahasiswa.index')->withErrors(['msg' => 'Gagal hapus data']);
        }
    }
}
